...
class BiEncoder(PreTrainedModel):
    ...
    def calc_dpp_loss(
        ...
        """
        print(f"q_vectors:{q_vectors}{q_vectors.shape}")
        print(f"ctx_vectors:{ctx_vectors}{ctx_vectors.shape}")
        print(f"ctx_indices:{ctx_indices}{ctx_indices}")
        ...
        print(f"kernel_matrix:{kernel_matrix}, {kernel_matrix.shape}")

        in_batch_neg_num = num_pos_neg
        scores = []
        for i in range(batch_size):
            per_sample_kernel_matrix = kernel_matrix[i]
            #print(f"per_sample_kernel_matrix:{per_sample_kernel_matrix}, {per_sample_kernel_matrix.shape}")
            # num_pos_neg, num_ice   ignore in-batch negative
            per_sample_ctx_indices = ctx_indices[i].reshape(-1, num_ice)
            #print(f"per_sample_ctx_indices:{per_sample_ctx_indices}", {per_sample_ctx_indices.shape})
            # num_pos_neg, num_ice, num_ice
            per_sample_neg_submatrix = indexing(per_sample_kernel_matrix, per_sample_ctx_indices)
            #print("############################################")
            #print(f"per_sample_neg_submatrix:{per_sample_neg_submatrix}, {per_sample_neg_submatrix.shape}")
            # num_pos_neg
            per_sample_scores = torch.linalg.slogdet(per_sample_neg_submatrix).logabsdet
            #print(f"per_sample_scores:{per_sample_scores}, {per_sample_scores.shape}")
...
def create_indices(indices):
    ...
    print(f"####################### indices1:{indices1}{indices1.shape}\nindices2:{indices2}{indices2.shape}")
